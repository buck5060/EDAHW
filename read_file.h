...
int fileReading(string netlist_filename)
{
	...
	while( getline(netlist,str_temp, ' ') )
	{
		...
		if(input_idx != 0) //new, using middle line
		{
			...
			{	
				//cout << "." ;
				if(trans[i].id == input_idx)
				{

					//cout << "exist trans for in1 found" << endl;
			...
			if (input_idx!= 0) //middle line doesn't exist
			{
				//cout << "using new trans" << endl;
				gates[gate_i].in1 = &trans[trans_i];
				trans[trans_i].to = &gates[gate_i];
				gates[gate_i].in1 -> id = input_idx;
				//cout << "trans id: " << trans[trans_i].id << endl;
				trans_i++;
				input_idx = 0;
			}
			...
			////it will not show the exist line assignment //cout << endl << ">>> middle line as input1 of this gate(on mem):" << trans[trans_i].to ->type << endl ;
			//cout << "input as in1 of this gate:" << gates[gate_i].in1 -> id << endl ;
		}
		...
		if(input_idx2 != 0) //check if input_idx2 is a input 
		{
			...
			if(input_idx2 != 0) //new, using middle line
			{
				...
				//cout << "temp: " << temp << endl;
				//cout << "trans_i: " << trans_i << endl;
				//cout << "input_idx2: " << input_idx2 << endl;
				...
				//cout << endl << ">>> middle line as input2 of this gate(on mem):" << trans[trans_i - 1].to ->type << endl ;
